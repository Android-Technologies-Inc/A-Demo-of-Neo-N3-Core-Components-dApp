...
```json
{
  ...
  "issuerPublicKeys": [
    "03b1e60e7d8a751092a8e36b1b95cb0cb64b44f2c82668749e9a4e3f16ff0b138c"
  ],
...
![generate_DID](IMAGE/generate_DID.png)

When DID is successfully generated, you can click on the icon <img src="IMAGE/button.png" alt="button" style="zoom:50%;" /> to see the generated DID.

![show_DID](IMAGE/show_DID.png)
...
generateDID = async (value: any) => {
  ...
  if (did) {
    ...
    value.nextTip(
      `Act as ${Agents.owner} and ask the digital Passport to the ${Agents.government}`
    );
  } else {
    value.changeAction("demoOwnerDID", "failure");
    value.nextTip(
      `Error occurred while generating the DID. Please go back to the Help Page, click the reset button and try again!`
    );
  }
};
```
...
![apply_for_passport](IMAGE/apply_for_passport.png)
...
![passport_request](IMAGE/passport_request.png)
...
getCredentials = (value: any) => {
  ...
  setTimeout(() => {
    value.nextTip(
      `Play as ${Agents.government} to issue credentials to ${Agents.owner}`
    );
...
![go_to_government](IMAGE/go_to_government.png)

![passport_requests_overview](IMAGE/passport_requests_overview.png)
...
issueCredential = (value: any, request: PassportReq) => {
  ...
  const govIssuer = new SeraphIDIssuer(
    configs.GOVERNMENT_SCRIPT_HASH,
    configs.NEO_RPC_URL,
    configs.DID_NETWORK,
    configs.MAGIC
  );
  ...
  const claimID = uuid();
  const newClaim = govIssuer.createClaim(
    claimID,
  ...
  govIssuer
    .issueClaim(newClaim, configs.GOVERNMENT_ISSUER_PRIVATE_KEY)
    .then((res) => {
      setTimeout(() => {
        ...
        try {
          ...
          value.nextTip(
            `Play as ${Agents.owner} and choose an accommodation from the ${Agents.smartAgency} Web Page.`
          );
...
![book_a_flat](IMAGE/book_a_flat.png)
...
![accommodation](IMAGE/accommodation.png)
...
![share_credential](IMAGE/share_credential.png)
...
const topFlats: FlatCard[] = [
  {
    id: 1,
    pictureRef: configs.OBJECT_ID1,
    city: "Florence",
    price: tokenPrice === 0 ? "" : GetTokenPrice(200, tokenPrice),
    rooms: 2,
    meters: 65,
    others: "Bathtub",
  },
  {
    id: 2,
    pictureRef: configs.OBJECT_ID2,
    city: "Beijing",
    price: tokenPrice === 0 ? "" : GetTokenPrice(280, tokenPrice),
    rooms: 3,
    meters: 110,
    others: "Garden",
  },
  {
    id: 3,
    pictureRef: configs.OBJECT_ID3,
    city: "New York",
    price: tokenPrice === 0 ? "" : GetTokenPrice(370, tokenPrice),
    rooms: 4,
    meters: 155,
    others: "Terrace",
  },
...
const bottomFlats: FlatCard[] = [
  {
    id: 4,
    pictureRef: configs.OBJECT_ID4,
    city: "Paris",
    price: tokenPrice === 0 ? "" : GetTokenPrice(195, tokenPrice),
    rooms: 2,
    meters: 54,
    others: "Position",
  },
  {
    id: 5,
    pictureRef: configs.OBJECT_ID5,
    city: "Zürich",
    price: tokenPrice === 0 ? "" : GetTokenPrice(450, tokenPrice),
    rooms: 5,
    meters: 240,
  ...
  {
    id: 6,
    pictureRef: configs.OBJECT_ID6,
    city: "Madrid",
    price: tokenPrice === 0 ? "" : GetTokenPrice(210, tokenPrice),
    rooms: 2,
    meters: 80,
    others: "Pool",
  },
...
<div className="PageContainer">
  ...
  <Fab
    ...
    style={this.style}
    onClick={() => {
      this.doNotShareCredentials(value);
    }}
  >
    {" "}
    Don't Share{" "}
...
shareCredentials = (value: any) => {
  ...
  if (passportClaimID) {
    ...
    if (passportClaim) {
      ...
      value.nextTip(
        `As ${Agents.smartAgency}, you need to verify the identity of ${Agents.owner} from your Web Page`
      );
...
1. In the agency card, click `GO TO ACCOMMODATION DAPP` on the right.
   ...
   ![go_to_accommodation](IMAGE/go_to_accommodation.png)
...
2. Click `VERIFY PASSPORT` to check the passport claim obtained from Oliver.
   ...
   ![verify_passport](IMAGE/verify_passport.png)
...
3. When the verification is passed, click `ISSUE` to issue the access key to Oliver. See [codes related to step 3](#related-to-step-3)
   ...
   ![issue_access_key](IMAGE/issue_access_key.png)
...
![verify_access_key](IMAGE/verify_access_key.png)
